...
namespace ManagementSystem
{
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            ...
            int i = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("Select orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price, total FROM tbOrder AS O JOIN tbCustomer AS C ON O.cid=C.cid JOIN tbProduct AS P ON O.pid=P.pid WHERE CONCAT ( orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price)LIKE '%" + textSearch.Text + "%' ", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
